...
char *get_nth_history(int index, int history_count, int history_numbers,
                      char **input_history);
void create_child_process(bool is_background, char **commands);
...
void print_history(int history_count, int history_numbers,
  ...
  for (int i = 0; i < history_count; i++) {
    char history_buf[MAX_COMMAND_SIZE];
    snprintf(history_buf, MAX_COMMAND_SIZE, "%d \t %s", index,
             input_history[i]);
  ...
  if (strcmp(commands[0], "exit") == 0) {
    ...
    if (strcmp(commands[0], "!!") == 0) {
      if (history_count < 1) {
        ...
        write(STDOUT_FILENO, last_line, strlen(last_line));

        tokenize_input(last_line, commands, &is_background);
        create_child_process(is_background, commands);
        add_to_history(last_line, &history_count, &history_numbers,
                       input_history);
        return;
      }
    } else {
      ...
      if (*end != '\0' || index < 0 || index >= history_count) {
        char *history_no_last = FORMAT_MSG("history", HISTORY_INVALID_MSG);
        write(STDERR_FILENO, history_no_last, strlen(history_no_last));
        return;
      }

      char *target_line = input_history[index];

      for (int i = 0; i < MAX_COMMAND_SIZE; i++) {
        commands[i] = NULL;
      }

      write(STDOUT_FILENO, target_line, strlen(target_line));

      tokenize_input(target_line, commands, &is_background);
      create_child_process(is_background, commands);

      add_to_history(target_line, &history_count, &history_numbers,
                     input_history);
      return;
    }
    // pid_t pid = fork();
